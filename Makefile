...
INSTALL			= /usr/local/ns-allinone-2.35/tcl8.5.10/unix/../unix/install-sh -c
...
RANLIB			= ranlib
DTRACE			= 
SRC_DIR			= /usr/local/ns-allinone-2.35/tcl8.5.10/unix
TOP_DIR			= $(SRC_DIR)/..
BUILD_DIR		= .
...
DLTEST_DIR		= /usr/local/ns-allinone-2.35/tcl8.5.10/unix/dltest
# Must be absolute to so the corresponding tcltest's tcl_library is absolute.
TCL_BUILDTIME_LIBRARY	= /usr/local/ns-allinone-2.35/tcl8.5.10/library
...
APP_CC_SWITCHES = $(CC_SWITCHES) 

LIBS		= -ldl  -lm

DEPEND_SWITCHES	= ${CFLAGS} -I${UNIX_DIR} -I${GENERIC_DIR} \
${AC_FLAGS} ${PROTO_FLAGS} ${EXTRA_CFLAGS} 
...
${LIB_FILE}: ${OBJS} ${STUB_LIB_FILE}
	rm -f $@
	${SHLIB_LD} -o $@ ${OBJS} ${SHLIB_LD_LIBS} ${TCL_SHLIB_LD_EXTRAS} ${TK_SHLIB_LD_EXTRAS} ${LD_SEARCH_FLAGS}
...
${TCL_EXE}: ${TCLSH_OBJS} ${TCL_LIB_FILE}
	${CC} ${CFLAGS} ${LDFLAGS} ${TCLSH_OBJS} -L/home/lexi/Documents/Thesis/ns-allinone-2.35 -ltcl8.5 ${LIBS}  \
...
tcltest-real:
	${CC} ${CFLAGS} ${LDFLAGS} ${TCLTEST_OBJS} -L/home/lexi/Documents/Thesis/ns-allinone-2.35 -ltcl8.5 ${LIBS}  \
...
install-binaries: binaries
	...
	@echo "Installing ${TCL_EXE} as $(BIN_INSTALL_DIR)/tclsh$(VERSION)"
	@$(INSTALL_PROGRAM) ${TCL_EXE} "$(BIN_INSTALL_DIR)"/tclsh$(VERSION)
...
clean:
	rm -f *.a *.o libtcl* core errs *~ \#* TAGS *.E a.out \
		errors ${TCL_EXE} tcltest lib.exp Tcl 
...
$(TCL_OBJS) $(STUB_LIB_OBJS) $(TCLSH_OBJS) $(TCLTEST_OBJS) $(XTTEST_OBJS): 
...
xttest:  ${XTTEST_OBJS} ${GENERIC_OBJS} ${UNIX_OBJS} ${COMPAT_OBJS} \
	...
	${CC} ${XTTEST_OBJS} ${GENERIC_OBJS} ${UNIX_OBJS} ${COMPAT_OBJS} \
		tclLoadDl.o -L/home/lexi/Documents/Thesis/ns-allinone-2.35 -ltcl8.5 ${LIBS} \
